...
class WGFVITON(nn.Module):
    def __init__(self, opt, item_nc, model_nc, output_nc, ngf=64, norm_G = 'spectralinstance', norm_layer=nn.BatchNorm2d, target_height=256):
        ...
        self.SPADE_blk = nn.ModuleList([
            PAFResBlock(ngf * 4 +ngf * 4 + self.ref_nChannel, ngf * 4, self.ref_input_nChannel, norm_G),
            PAFResBlock(ngf * 4 +ngf * 4 + self.ref_nChannel, ngf * 4, self.ref_input_nChannel, norm_G),
            PAFResBlock(ngf * 4 +ngf * 4 + self.ref_nChannel, ngf * 4, self.ref_input_nChannel, norm_G),
            PAFResBlock(ngf * 4 +ngf * 4 + self.ref_nChannel, ngf * 2, self.ref_input_nChannel, norm_G, use_mask_norm=False),
            PAFResBlock(ngf * 2 +ngf * 4 + self.ref_nChannel, ngf * 1, self.ref_input_nChannel, norm_G, use_mask_norm=False)
        ])                          
